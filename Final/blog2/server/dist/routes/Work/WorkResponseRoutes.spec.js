...
describe("POST /work_resp", () => {
    it("get response to a work", async () => {
        const author = await repo.Profile.insertProfile(faker.internet.username(), faker.internet.password(), faker.internet.displayName(), faker.lorem.sentence(2), faker.internet.url(), faker.internet.url(), avatars[0]);
        ...
        for (let i = 0; i < 10; i++) {
            ...
            await repo.WorkResp.insertWorkResponse(work.id, responder.id, responses[i]);
        }
...
describe("POST /work_resp_author", () => {
    it("get work responses by author", async () => {
        ...
        const firstFive = await repo.WorkResp.selectWorkResponsesByAuthor(responder.id, 5);
        const lastCursor = firstFive[firstFive.length - 1].id;
        await request(app)
