...
describe("POST /work/update", () => {
    it("update work", async () => {
        ...
        await request(app)
            ...
            assert.equal(comparisonWork?.workTopics
                .map((wt) => serializeBigInt(wt.topic.id))
                .includes(topicaId), false);
            assert.equal(comparisonWork?.workTopics
                .map((wt) => serializeBigInt(wt.topic.id))
                .includes(topicbId), true);
        });
    });
});
describe("GET /work/:id", () => {
    it("get work", async () => {
        const profile = await repo.Profile.insertProfile(faker.internet.username(), faker.internet.password(), faker.internet.displayName(), faker.lorem.sentence(2), faker.internet.url(), faker.internet.url(), avatars[0]);
        ...
        const work = await repo.Work.insertWork(title, description, content, profile.id, [topic.id], [
            {
                imagePlaceholder: "A",
                image: avatars[0],
            },
...
describe("POST /work_popular", () => {
    it("get popular work by topic", async () => {
        ...
        await request(app)
            ...
            .expect(200)
            .then(async (res) => {
            const popularWorks = res.body;
            assert.equal(popularWorks[0].workLikes.length >
                popularWorks[popularWorks.length - 1].workLikes.length, true);
        });
    });
});
describe("POST /work_latest", () => {
    it("get latest work", async () => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const description = faker.lorem.sentence(2);
            const content = faker.lorem.sentence(4);
            await repo.Work.insertWork(title, description, content, profile.id, [topic.id], [
                {
                    imagePlaceholder: "A",
                    image: avatars[0],
                },
        ...
        await request(app)
            ...
            //.expect(200)
            .then(async (res) => {
            const latestWorks = res.body;
            assert.equal(latestWorks[0].updatedAt >
                latestWorks[latestWorks.length - 1].updatedAt, true);
        });
    });
});
describe("POST /work_followed", () => {
    it("get followed works", async () => {
        ...
        for (let i = 0; i < followedCount; i++) {
            const followed = await repo.Profile.insertProfile(faker.internet.username(), faker.internet.password(), faker.internet.displayName(), faker.lorem.sentence(3), faker.internet.url(), faker.internet.url(), getAvatar());
