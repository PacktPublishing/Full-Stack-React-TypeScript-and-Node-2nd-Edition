...
describe("Repository Profile", () => {
    it("Create a profile and login successfully with it", async () => {
        ...
        await repo.Profile.insertProfile(userName, password, fullName, desc, primaryUrl, secondaryUrl, avatar);
    ...
    it("Create a profile and confirm its fields", async () => {
        ...
        const primaryUrl = faker.internet.url();
        const secondaryUrl = faker.internet.url();
        const profile = await repo.Profile.insertProfile(userName, password, fullName, desc, primaryUrl, secondaryUrl, avatar);
    ...
    it("Create 10 Profiles with likes", async () => {
        ...
        const works = new Array(count);
        for (let i = 0; i < count; i++) {
            authors[i] = await repo.Profile.insertProfile(faker.internet.username(), faker.internet.password(), faker.internet.displayName(), faker.lorem.sentence(5), faker.internet.url(), faker.internet.url(), getAvatar());
            const title = faker.lorem.sentence(6);
            const description = faker.lorem.sentence(10);
            const content = faker.lorem.sentences(2);
            works[i] = await repo.Work.insertWork(title, description, content, authors[i].id, []);
        }
        let likesToInsert = 10;
        for (let i = 0; i < count; i++) {
            for (let l = 0; l < likesToInsert; l++) {
                await repo.WorkLikes.insertWorkLike(works[i].id, authors[i + 1 >= count - 1 ? 0 : i + 1].id);
            }
            likesToInsert -= 1;
        }
    ...
    it("Create Profile; Update Profile; and confirm its updated fields", async () => {
        ...
        let createdAuthor = await repo.Profile.insertProfile(userName, password, fullName, desc, primaryUrl, secondaryUrl, avatar);
        avatar = getAvatar();
        ...
        await repo.Profile.updateProfile(createdAuthor.id, fullName, password, desc, primaryUrl, secondaryUrl, avatar);
    ...
    it("Create Profile; get it back; and confirm its avatar", async () => {
        const { repo, cleanup } = await createClientAndTestDb();
        const avatar = getAvatar();
        const profile = await repo.Profile.insertProfile(faker.internet.username(), faker.internet.password(), faker.internet.displayName(), faker.lorem.sentence(5), faker.internet.url(), faker.internet.url(), avatar);
        const selectedProfile = await repo.Profile.selectProfile(profile.id);
        const avatarResult = await repo.ProfileAvatar.selectProfileAvatar(selectedProfile?.avatarId);
