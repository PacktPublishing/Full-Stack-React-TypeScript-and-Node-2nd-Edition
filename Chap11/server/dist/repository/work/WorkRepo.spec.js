...
describe("Work tests", () => {
    it("insertWork creates a new work", async () => {
        ...
        const author = await repo.Profile.insertProfile(faker.internet.username(), faker.internet.password(), faker.internet.displayName(), faker.lorem.sentence(5), faker.internet.url(), faker.internet.url(), avatar);
    ...
    it("updateWork updates an existing work", async () => {
        ...
        const work = await repo.Work.insertWork(title, description, content, author.id, [topica.id] // work created with topica
        );
        ...
        await repo.Work.updateWork(work.id, updateTitle, updateDesc, updateContent, [topicb.id] // work updated where topica is removed and topicb is added
        );
        ...
        assert.equal(workTopics.filter((wt) => wt.topicId === topica.id)?.length, 0);
        assert.equal(workTopics.filter((wt) => wt.topicId === topicb.id)?.length, 1);
        cleanup();
    });
    it("insertWork creates a new work with one image", async () => {
        ...
        const workImage = await repo.WorkImage.selectWorkImage(work.id, imagePlaceholder);
        assert.equal(work.title, title);
        assert.equal(work.description, description);
        assert.equal(work.content, content);
        assert.equal(work.authorId, author.id);
    ...
    it("selectWork, gets work with author and correct likes", async () => {
        ...
        const author = await repo.Profile.insertProfile(userName, password, fullName, desc, faker.internet.url(), faker.internet.url(), avatar);
        const topic = await repo.Topic.insertTopic(faker.company.name());
        const newWork = await repo.Work.insertWork(title, description, content, author.id, [topic.id]);
    ...
    it("selectMostPopularWorks, gets all works by most likes", async () => {
        ...
        let likeCount = 10;
        for (let i = 0; i < 10; i++) {
            const newWork = await repo.Work.insertWork(title, description, content, author.id, []);
    ...
    it("selectMostPopularWorks, gets works with most likes by topic", async () => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            newWork[i] = await repo.Work.insertWork(title, description, content, author.id, [topics[topicId].id]);
            for (let y = 0; y < likeCount; y++) {
        ...
        assert.equal(works[0].workLikes.length >= works[works.length - 1].workLikes.length, true);
        assert.equal(works.every((w) => w.workTopics.every((wt) => wt.id === testingTopicId)), true);
        cleanup();
    });
    it("selectLatestWorksByAuthor, gets works by author desc", async () => {
        ...
        works = await repo.Work.selectLatestWorksByAuthor(author.id, -5, works[works.length - 1].id);
    ...
    it("selectWorksOfFollowed, gets works by follower's followed users desc", async () => {
        ...
        const fullName = faker.internet.displayName();
        const desc = faker.lorem.sentence(5);
        const follower = await repo.Profile.insertProfile(userName, password, fullName, desc, faker.internet.url(), faker.internet.url(), avatar);
        const followedCount = 10;
        const followedWorkIds = [];
        for (let i = 0; i < followedCount; i++) {
            ...
            for (let i = 0; i < 5; i++) {
                const followedWork = await repo.Work.insertWork(title, description, content, followed.id, []);
        ...
        const nextFive = await repo.Work.selectWorksOfFollowed(follower.id, 5, lastCursor);
    ...
    it("selectWorksOfOneFollowed, gets works of followed profile", async () => {
        ...
        const lastCursor = firstFive[firstFive.length - 1].id;
        const nextFive = await repo.Work.selectWorksOfOneFollowed(followed.id, 5, lastCursor);
        const reversedWorkIds = followedWorkIds.reverse();
        assert.equal(nextFive[0].id, reversedWorkIds[5]);
        assert.equal(nextFive[nextFive.length - 1].id, reversedWorkIds[9]);
        cleanup();
    ...
    it("selectWorksByTopic, gets works by topic", async () => {
        ...
        const topicWorkIds = [];
        for (let i = 0; i < 10; i++) {
            topicWorkIds.push((await repo.Work.insertWork(title, description, content, author.id, [
                topic.id,
            ])).id);
        }
        const firstFive = await repo.Work.selectWorksByTopic(topic.id, 5);
        const workIdCursor = firstFive[firstFive.length - 1].id;
        const nextFive = await repo.Work.selectWorksByTopic(topic.id, 5, workIdCursor);
    ...
    it("searchWorks, gets works by search text", async () => {
        ...
        for (let i = 0; i < 3; i++) {
            topicWorkIds.push((await repo.Work.insertWork(title, title + description, content, author.id, [topic.id])).id);
        }
        for (let i = 0; i < 7; i++) {
            topicWorkIds.push((await repo.Work.insertWork(faker.lorem.sentence(1), faker.lorem.sentence(2), faker.lorem.sentence(3), author.id, [topic.id])).id);
        }
