...
describe("Repository Like", () => {
    it("Create like and verify", async () => {
        ...
        const work = await repo.Work.insertWork(faker.lorem.sentence(6), faker.lorem.sentence(10), faker.lorem.sentences(2), author.id, [topic.id]);
        const liker = await repo.Profile.insertProfile(faker.internet.username(), faker.internet.password(), faker.internet.displayName(), faker.lorem.sentence(5), faker.internet.url(), faker.internet.url(), getAvatar());
